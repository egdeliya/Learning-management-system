...
class AdminServiceAddTeacherToCourseTest extends FlatSpec
  ...
  "AdminService" should "add teacher to course successfully" in new init {
  ...
  "AdminService" should "fail adding fake teacher to course" in new init {
    val fakeTeacherId = 0
    val result = createCourses()
      .flatMap {_ =>
  ...
  "AdminService" should "fail adding teacher to fake course" in new init {
    val fakeCourse = "bla"
  ...
  "AdminService" should "fail adding fake teacher to fake course" in new init {
    val fakeCourse = "bla"
    val fakeTeacherId = 0
