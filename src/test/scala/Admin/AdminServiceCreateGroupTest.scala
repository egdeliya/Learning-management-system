...
class AdminServiceCreateGroupTest extends FlatSpec
  ...
  "Admin.AdminService" should "create group successfully" in new init {
  ...
  "Admin.AdminService" should "fail creating group with empty name" in new init {
    val result = Future.sequence(groups.map { group =>
  ...
  "Admin.AdminService" should "fail creating group with empty faculty" in new init {
    val result = Future.sequence(groups.map { group =>
  ...
  "Admin.AdminService" should "fail creating group with negative course number" in new init {
    val result = Future.sequence(groups.map { group =>
